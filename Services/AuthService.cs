...
namespace Codeline_HealthCareCenter_OOP.Services
{
    public class AuthService : IAuthService // Implementation of IAuthService
    {
        ...
        public Task SaveTokenToCookie(string token)
        {
            ...
            {
                Directory.CreateDirectory(directory); //  only if valid directory exists
            }
            ...
            File.WriteAllText(TokenFilePath, token); //  safe to write
            return Task.CompletedTask;
        }
        ...
        public Task<UserInputDTO?> Login(string email, string password)
        {
            ...
            if (email.Equals("SA", StringComparison.OrdinalIgnoreCase) && password == "123") // Check for SuperAdmin credentials
                {
                return Task.FromResult<UserInputDTO?>(new UserInputDTO
                {
                    Email = email,
                    Password = password,
                    Role = "superadmin",
